...
static int rv1106_clk_enable(struct clk *clk)
{
	switch (clk->id) {
	case TCLK_WDT_NS:
	case PCLK_WDT_NS:
...
U_BOOT_DRIVER(rockchip_rv1106_cru) = {
	...
	.of_to_plat = rv1106_clk_ofdata_to_platdata,
	.ops		= &rv1106_clk_ops,
...
static ulong rv1106_grfclk_get_rate(struct clk *clk)
{
	...
	ret = uclass_get_device_by_driver(UCLASS_CLK,
					  DM_DRIVER_GET(rockchip_rv1106_cru),
					  &cru_dev);
	if (ret) {
...
U_BOOT_DRIVER(rockchip_rv1106_grf_cru) = {
	...
	.priv_auto  = sizeof(struct rv1106_grf_clk_priv),
	.of_to_plat = rv1106_grfclk_ofdata_to_platdata,
